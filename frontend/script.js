class VideoDownloader {
    ...
    async handleDownload(e) {
        ...
        // Platform-spezifische URL-Validierung
        if (!this.isPlatformUrlValid(url, platform)) {
            this.showError(`Diese URL gehört nicht zu ${platform}. Bitte überprüfen Sie die Plattform-Auswahl.`);
            return;
        }
    ...
    isPlatformUrlValid(url, platform) {
        ...
        switch (platform) {
            ...
            case 'snapchat':
                return urlLower.includes('snapchat.com');
            case 'pinterest':
                return urlLower.includes('pinterest.com');
            case 'amazon':
                return urlLower.includes('amazon.');
            default:
                return true;
        }
    }
    ...
    async makeDownloadRequest(url, platform) {
        ...
        try {
            ...
            // Automatischer Download nur bei Erfolg
            setTimeout(() => {
            ...
            if (error.name === 'AbortError') {
                return {
                    status: "error",
                    message: "Request-Timeout: Der Server braucht zu lange für die Antwort. Bitte versuchen Sie es erneut."
                };
            }
    ...
    startLoading(loadingEl, downloadBtn) {
        ...
        }, 1200); // Langsamere Nachrichten-Rotation
